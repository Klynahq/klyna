    ...
    >
      <section style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        <p style={{ color: '#c7c7d1', fontSize: 18, lineHeight: 1.6, margin: '0 0 32px' }}>
          This app now keeps Free available in-app, implements Pro as a first-class Shopify Billing
          API plan, requests the selected paid plan, and displays the active plan returned by
        ...
        >
          {[
            ['Plans', 'Free stays available in-app; Pro is configured in Shopify Billing.'],
            ['Request flow', 'The Pro button submits the Pro plan name to billing.request().'],
            [
              'Return flow',
            ...
            [
              'Active state',
              'The billing screen reads active subscriptions, treats legacy starter charges as Pro, and labels Pro as Active plan.',
            ],
            [
              'Downgrade flow',
              'Paid users can choose Downgrade to Free, which cancels the active Shopify subscription.',
            ],
          ].map(([title, body]) => (
            <article
              key={title}
