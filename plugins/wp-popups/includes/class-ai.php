...
final class Ai {
	...
	private const CACHE_PREFIX = 'wp_popups_ai_cache_';
	private const CACHE_TTL    = DAY_IN_SECONDS;
	private const DAILY_KEY    = 'wp_popups_ai_count_today';
	...
	/**
	 * Test the configured provider with a tiny prompt.
	 * Returns ['ok' => bool, 'text' => string, 'reason'? => string].
	 */
	public static function test(): array {
		$ai     = new self();
		$result = $ai->complete( 'Reply with the single word: ok', array( 'max_tokens' => 8, 'temperature' => 0.0 ) );
		return $result;
	}
