...
final class Ai {
	...
	private const CACHE_PREFIX = 'wp_speed_ai_cache_';
	private const CACHE_TTL    = DAY_IN_SECONDS;
	private const DAILY_KEY    = 'wp_speed_ai_count_today';
	private const SYSTEM_PROMPT = "You are an expert WordPress performance engineer. Output ONLY the requested content with no preamble, explanation, or markdown code fences. Be specific and factual; never invent statistics.";
	...
	private function openai_compatible(
		...
		if ( '' === $api_key ) {
			return array( 'ok' => false, 'reason' => 'missing_api_key', 'text' => 'API key not set in Klyna Speed Settings.' );
		}
		$headers = array_merge(
			array(
	...
	/**
	 * Lightweight connection test: sends a 1-token ping to the configured provider.
	 *
	...
	public function test(): array {
		...
		if ( '' === $provider || 'off' === $provider ) {
			return array( 'ok' => false, 'reason' => 'disabled', 'text' => 'AI assistant is off.' );
		}
		$model    = (string) $settings['ai_model'];
		$key      = (string) $settings['ai_api_key'];
		$endpoint = (string) $settings['ai_endpoint'];
		$result = $this->call_provider( $provider, $model, $key, $endpoint, 'Reply with the single word OK.', 0.0, 8 );
		return $result;
	}
