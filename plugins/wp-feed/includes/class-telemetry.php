...
final class Telemetry {
    ...
    public const OPTION_KEY     = 'klyna_wp_feed_telemetry_enabled';
    public const LAST_PING_KEY  = 'klyna_wp_feed_telemetry_last_pinged';
    public const SETTINGS_GROUP = 'klyna_telemetry_group_wp_feed';
    public const ENDPOINT       = 'https://klyna.dev/api/track/install';
    public const SLUG           = 'wp-feed';
    ...
    public static function maybe_ping(): void {
        ...
        $body = array(
            'slug'       => self::SLUG,
            'kind'       => 'wp',
            'version'    => defined( 'KLYNA_FEED_VERSION' ) ? KLYNA_FEED_VERSION : '0.0.0',
    ...
    public static function render_form(): void {
        ...
        <form method="post" action="options.php" class="card klyna-telemetry-card" style="margin-top:1rem;max-width:none;padding:1rem 1.25rem;">
            ...
            <h2 style="margin-top:0;font-size:14px;text-transform:uppercase;letter-spacing:0.05em;color:#71717a;">
                <?php esc_html_e( 'Anonymous install stats', 'wp-feed' ); ?>
            </h2>
            <label style="display:flex;gap:0.6rem;align-items:flex-start;">
                ...
                <span>
                    <strong><?php esc_html_e( 'Share anonymous install stats (hashed site ID + version only)', 'wp-feed' ); ?></strong><br>
                    <span class="description">
                        <?php esc_html_e( 'Helps Klyna know which plugins are getting used. Default: off. No personal data — only a hashed site identifier and plugin/WP/PHP version. Pinged at most once per 7 days.', 'wp-feed' ); ?>
                    </span>
                </span>
            </label>
            <p><?php submit_button( __( 'Save telemetry preference', 'wp-feed' ), 'secondary', 'submit', false ); ?></p>
        </form>
        <?php
    }
}
