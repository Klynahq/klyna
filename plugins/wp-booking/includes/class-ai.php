...
final class Ai {
	...
	private const CACHE_PREFIX = 'wp_booking_ai_cache_';
	private const CACHE_TTL    = DAY_IN_SECONDS;
	private const DAILY_KEY    = 'wp_booking_ai_count_today';
	...
	public static function is_enabled(): bool {
		...
		return array_key_exists( $provider, self::provider_catalog() );
	}

	/** Lightweight connectivity check. Sends a tiny prompt to verify creds. */
	public static function test(): array {
		$ai     = new self();
		$result = $ai->complete( 'Reply with the single word: ok.', array( 'max_tokens' => 8, 'temperature' => 0 ) );
		return $result;
	}
	...
	public function complete( string $user_prompt, array $opts = array() ): array {
		...
		if ( 'off' === $provider || '' === $provider ) {
			return array( 'ok' => false, 'reason' => 'disabled', 'text' => 'AI assistant is off in Booking Settings.' );
		}
