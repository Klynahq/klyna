...
final class Ai {
	...
	private const CACHE_PREFIX = 'wp_tables_ai_cache_';
	private const CACHE_TTL    = DAY_IN_SECONDS;
	private const DAILY_KEY    = 'wp_tables_ai_count_today';
	...
	public function complete( string $user_prompt, array $opts = array() ): array {
		...
		if ( 'off' === $provider || '' === $provider ) {
			return array( 'ok' => false, 'reason' => 'ai_disabled', 'text' => 'AI is disabled in Klyna Tables settings.' );
		}
		$catalog       = self::provider_catalog();
		$default_model = $catalog[ $provider ]['default_model'] ?? '';
		$configured    = (string) ( $settings['ai_model'] ?? '' );
		$model         = $opts['model'] ?? ( '' !== $configured ? $configured : $default_model );
	...
	private function openai_compatible(
		...
		if ( '' === $api_key ) {
			return array( 'ok' => false, 'reason' => 'missing_api_key', 'text' => 'API key not set in Klyna Settings.' );
		}
		$headers = array_merge(
			array(
	...
	/**
	 * Quick connectivity test — runs a tiny prompt against the configured provider.
	 *
	 * @return array{ok:bool,text:string,reason?:string}
	 */
	public function test(): array {
		return $this->complete( 'Reply with the single word OK.', array( 'max_tokens' => 8, 'temperature' => 0.0 ) );
	}
