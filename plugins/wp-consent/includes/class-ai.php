...
final class Ai {
	...
	private const CACHE_PREFIX = 'wp_consent_ai_cache_';
	private const CACHE_TTL    = DAY_IN_SECONDS;
	private const DAILY_KEY    = 'wp_consent_ai_count_today';
	private const SYSTEM_PROMPT = "You are a privacy and compliance writer producing GDPR / ePrivacy cookie policies for WordPress sites. Output ONLY the requested content with no preamble, explanation, or markdown code fences. Be specific and factual; never invent statistics, quotes, or legal citations. Use clear, plain language appropriate for end users.";
	...
	/**
	 * Quick check used by the UI / killer feature: do we have a working config?
	 */
	public static function is_configured(): bool {
		$s = array_merge( self::default_settings(), Plugin::settings() );
		if ( empty( $s['ai_provider'] ) || $s['ai_provider'] === 'off' ) {
			return false;
		}
		$catalog = self::provider_catalog();
		if ( ! isset( $catalog[ $s['ai_provider'] ] ) ) {
			return false;
		}
		foreach ( (array) $catalog[ $s['ai_provider'] ]['requires'] as $req ) {
			if ( empty( $s[ $req ] ) ) {
				return false;
			}
		}
	...
	/**
	 * Lightweight live test — performs a single-token completion.
	 * Returns ['ok' => bool, 'text' => string, 'reason'? => string].
	 */
	public function test(): array {
		if ( ! self::is_configured() ) {
			return array( 'ok' => false, 'reason' => 'not_configured', 'text' => 'AI is not configured.' );
		}
		$client = new self();
		$res    = $client->complete( 'Reply with the single word: OK', array( 'max_tokens' => 8, 'temperature' => 0.0 ) );
		return $res;
	}
	...
	public function complete( string $user_prompt, array $opts = array() ): array {
		...
		if ( '' === $provider || 'off' === $provider ) {
			return array( 'ok' => false, 'reason' => 'ai_disabled', 'text' => 'AI assistant is disabled. Enable it in Klyna Consent settings.' );
		}
	...
	private function openai_compatible(
		...
		if ( '' === $api_key ) {
			return array( 'ok' => false, 'reason' => 'missing_api_key', 'text' => 'API key not set in Klyna Consent settings.' );
		}
		$headers = array_merge(
			array(
