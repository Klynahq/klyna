...
final class Ai {
	...
	private const CACHE_PREFIX = 'wp_redirects_ai_cache_';
	private const CACHE_TTL    = DAY_IN_SECONDS;
	private const DAILY_KEY    = 'wp_redirects_ai_count_today';
	private const SYSTEM_PROMPT = "You are an expert at matching broken URLs to their most likely intended destination on a website. Output ONLY the single best target path (starting with /) or full URL with no preamble, explanation, quotes, or markdown. If no candidate is a good match, output exactly: NONE";
	...
	public function complete( string $user_prompt, array $opts = array() ): array {
		$settings = array_merge( self::default_settings(), (array) get_option( KLYNA_REDIRECTS_OPTION_KEY, array() ) );
	...
	private function openai_compatible(
		...
		if ( '' === $api_key ) {
			return array( 'ok' => false, 'reason' => 'missing_api_key', 'text' => 'API key not set in WP Redirects settings.' );
		}
		$headers = array_merge(
			array(
